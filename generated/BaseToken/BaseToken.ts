...
export class BaseToken extends ethereum.SmartContract {
  static bind(address: Address): BaseToken {
    return new BaseToken("BaseToken", address);
  }

  BUILT_ON(): string {
    let result = super.call("BUILT_ON", "BUILT_ON():(string)", []);

    return result[0].toString();
  }

  try_BUILT_ON(): ethereum.CallResult<string> {
    let result = super.tryCall("BUILT_ON", "BUILT_ON():(string)", []);
    if (result.reverted) {
